...
//no he podido comprobar nada, el docker no me iba y ya habia perdido mucho tiempo

    public function __construct() {
        $this->tablero = array_fill(0, self::COLUMNAS, []);
        $this->jugadorActual = self::JUGADOR_1;
    ...
    public function InsertarValor(int $columna): bool {
        ...
        if ($jugadorQueInserta == 2) {
            ValorIA($jugadorActual);
        }else {
            ...
            if ($this->ComprobarGanador() === 0) {
                ...
                $cont ++;
                $this->ganadores[$cont] = $this->ultimoGanador 
                ContarGanadores($this->ganadores);
            }
        }
    ...
    public function ValorIA(int $jugador){
        $col = rand(0, self::COLUMNAS);
        while (!empty($this->tablero[$col])) {
            $col = rand(0, self::COLUMNAS);
        }
        $this->tablero[$col][] = $jugador;
            if ($this->ComprobarGanador() === 0) {
                $this->cambiarTurno();
            } else {
                $this->ultimoGanador = $jugador;
                $cont ++;
                $this->ganadores[$cont] = $this->ultimoGanador 
                ContarGanadores($this->ganadores);
            }
    ...
    private function cambiarTurno(): void {
        $this->jugadorActual = ($this->jugadorActual === self::JUGADOR_1) ? self:: IA : self::JUGADOR_1;
    }
...
if (!$juegoTerminado) {
    ...
    if (isset($_POST["btnIA"])) {
        $turnIA = (int)$_POST["btnIA"];

        $seInserto = $conecta->InsertarValor($turnIA);
...
$arrayGanadores = $conecta->Ganadores();
function BD($arrayGanadores){
    
        $j1 = 0;
        $ia = 0;
        $emp= 0;
        for ($x=0; $x <count($arrayGanadores) ; $x++) { 
            if ($arrayGanadores[$x] == 1) {
                $j1 ++;
            
            }elseif ($arrayGanadores[$x] == 2) {
                $ia ++;
            }else{
                $emp ++;
            }
        ...
        $sql = "INSERT INTO contecta4 (JUGADOR_1, JUGADOR_2, EMPATE) VALUES ($j1, $ia, $emp)";
        $conexion->exec($sql);
...
if (!$juegoTerminado) {
    ...
    for ($i = 0; $i < $columnas; $i++) {
        ...
        echo "<button type='submit' name='btnIA' value='" . $i . "' title='" . $titulo . "'" . $disabled_attr . ">IA</button>";
